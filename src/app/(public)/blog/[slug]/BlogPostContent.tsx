  ...
  return (
    <article className="max-w-4xl mx-auto py-12 sm:py-16 px-4 sm:px-6 lg:px-8">
      <div className="mb-8">
        <Link
          href="/#blog"
          className="inline-flex items-center gap-1.5 font-mono text-xs text-primary uppercase tracking-widest border border-border hover:border-primary px-4 py-2 rounded-sm transition-colors"
        >
      ...
      <div className="mb-10 sm:mb-12">
        {post.tags && post.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {post.tags.map((tag) => (
              <span
                key={tag}
                className="font-mono text-xs text-primary border border-border px-2 py-0.5 hover:border-primary hover:text-primary transition-colors"
              >
                #{tag}
              </span>
            ))}
        ...
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight text-foreground mb-6 leading-tight">
          {title}
        </h1>

        <div className="flex flex-wrap items-center gap-4 sm:gap-6 font-mono text-xs text-muted-foreground">
          {post.publishedAt && (
          ...
          <div className="flex items-center gap-1.5">
            <Clock className="w-3.5 h-3.5" />
            <span>{readingTime} {locale === "th" ? "นาที อ่าน" : "min read"}</span>
          </div>
        </div>
      </div>
